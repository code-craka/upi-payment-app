...
export async function GET(_request: NextRequest) {
  try {
    ...
    const [user, authData] = await Promise.all([currentUser(), auth()]);
    ...
    const debugInfo = {
      ...
      roleExtractionTests: {
        ...
        fromSessionClaims: (authData.sessionClaims as Record<string, Record<string, unknown>>)?.publicMetadata?.role,
        fromSessionClaimsMetadata: (authData.sessionClaims as Record<string, Record<string, unknown>>)?.metadata?.role,
        fromSessionClaimsCustom: (authData.sessionClaims as Record<string, Record<string, unknown>>)?.custom_claims?.role,
      },
