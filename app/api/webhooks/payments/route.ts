...
async function handlePaymentConfirmation(data: z.infer<typeof PaymentConfirmationSchema>['data']) {
  ...
  if (order.amount !== data.amount) {
    throw new Error(
      `Amount mismatch for order ${data.orderId}: expected ${order.amount}, received ${data.amount}`,
    );
  }
...
export async function POST(request: Request) {
  try {
    ...
    switch (data.event) {
      ...
      default:
        return NextResponse.json({ error: `Unknown event type: ${data.event}` }, { status: 400 });
    }
...
export async function GET() {
  return NextResponse.json({
    ...
    events: ['payment.confirmed', 'payment.failed', 'order.status_updated'],
  });
}
