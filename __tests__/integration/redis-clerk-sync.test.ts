...
describe('Redis Circuit Breaker Integration', () => {
  ...
  it('should recover to HALF_OPEN after timeout', async () => {
    ...
    const _result = await circuitBreaker.execute(async () => ({ success: true }));
    expect(circuitBreaker.getState()).toBe('CLOSED');
  });
});
...
describe('Batch Operations', () => {
  it('should batch invalidate multiple user roles', async () => {
    ...
    await Promise.all(userIds.map((userId) => cacheUserRole(userId, 'merchant')));

    // Batch invalidate
    await batchInvalidateRoles(userIds);

    // Verify all are invalidated
    const results = await Promise.all(userIds.map((userId) => getCachedUserRole(userId)));
