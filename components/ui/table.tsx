...
function createTableCellComponent<T extends 'th' | 'td'>(
  ...
  const Component = ({ className, ...props }: React.ComponentProps<T>) => {
    const Element = element as React.ElementType;
    return <Element data-slot={dataSlot} className={cn(baseClassName, className)} {...props} />;
  };
  Component.displayName = `Table${element.charAt(0).toUpperCase() + element.slice(1)}Component`;
  return Component;
}
