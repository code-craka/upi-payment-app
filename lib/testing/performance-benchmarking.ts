...
class PerformanceBenchmarkingSuite {
  ...
  public async benchmarkRedisVsClerk(config?: {
    ...
    console.log(
      `🚀 Starting Redis vs Clerk benchmark across ${regions.length} regions with ${iterations} iterations`,
    );
  ...
  public async validateCacheHitRatios(config?: {
    ...
    const patterns = config?.patterns || [
      ...
      },
      {
        ...
        userCount: 100,
        operationsPerUser: 50,
        accessPattern: 'hotspot' as const,
      },
      {
        name: 'Realistic Pattern',
        userCount: 200,
        operationsPerUser: 25,
        accessPattern: 'realistic' as const,
      },
    ];
  ...
  public async testConcurrentUsers(config?: {
    ...
    const raceDetectionData: Map<
      string,
      Array<{ timestamp: number; operation: string; value: string }>
    > = new Map();
  ...
  public async generatePerformanceReport(
    ...
    const report: PerformanceReport = {
      ...
      comparisonData: comparisonBaseline
        ? await this.generateComparisonData(comparisonBaseline, allMetrics)
        : undefined,
    };
  ...
  private recordBenchmark(
    metric: Omit<BenchmarkMetrics, 'operation'> & { operation?: string },
    testId: string,
  ): void {
    const testData = this.activeTests.get(testId);
    if (testData) {
  ...
  private async benchmarkRedisLookup(
    userId: string,
    region: string,
    testId: string,
  ): Promise<void> {
  ...
  private async benchmarkClerkLookup(
    userId: string,
    region: string,
    testId: string,
  ): Promise<void> {
  ...
  private async benchmarkHybridLookup(
    userId: string,
    region: string,
    testId: string,
  ): Promise<void> {
  ...
  private detectRaceConditions(
    raceData: Map<string, Array<{ timestamp: number; operation: string; value: string }>>,
  ) {
    ...
    switch (operation.type) {
      ...
      case 'user_creation':
        await redis.setex(
          `user:${userId}`,
          300,
          JSON.stringify({ id: userId, created: Date.now() }),
        );
        break;
  ...
  private generateInsights(metrics: BenchmarkMetrics[], percentiles: any): string[] {
    ...
    if (percentiles.p50 < 30) {
      insights.push(
        `✅ Median response time (${percentiles.p50.toFixed(2)}ms) meets sub-30ms target`,
      );
    } else {
      insights.push(
        `⚠️  Median response time (${percentiles.p50.toFixed(2)}ms) exceeds 30ms target`,
      );
    }

    if (percentiles.p99 > 100) {
      insights.push(
        `🚨 99th percentile response time (${percentiles.p99.toFixed(2)}ms) indicates performance issues for slowest requests`,
      );
    }
    ...
    if (cacheOps.length > 0) {
      ...
      if (hitRatio > 0.8) {
        ...
        insights.push(
          `⚠️  Moderate cache hit ratio: ${(hitRatio * 100).toFixed(1)}% - consider optimization`,
        );
      } else {
        insights.push(
          `🚨 Low cache hit ratio: ${(hitRatio * 100).toFixed(1)}% - requires immediate attention`,
        );
      }
    }
    ...
    if (redisOps.length > 0 && clerkOps.length > 0) {
      ...
      if (avgClerkTime > avgRedisTime * 3) {
        recommendations.push(
          'Clerk API calls are significantly slower than Redis - ensure proper caching strategy',
        );
      }
    }
  ...
  private async generateComparisonData(baselineId: string, currentMetrics: BenchmarkMetrics[]) {
    ...
    const currentPercentiles = this.calculatePercentiles(
      currentResponseTimes.sort((a, b) => a - b),
    );

    const improvement = {
      p50ResponseTime:
        ((baseline.summary.percentiles.p50 - currentPercentiles.p50) /
          baseline.summary.percentiles.p50) *
        100,
      p95ResponseTime:
        ((baseline.summary.percentiles.p95 - currentPercentiles.p95) /
          baseline.summary.percentiles.p95) *
        100,
      p99ResponseTime:
        ((baseline.summary.percentiles.p99 - currentPercentiles.p99) /
          baseline.summary.percentiles.p99) *
        100,
      errorRate:
        baseline.summary.failedOperations / baseline.summary.totalOperations -
...
export const runQuickBenchmark = () =>
  performanceBenchmark.runFullBenchmarkSuite({ includeLongRunningTests: false });

export const runComprehensiveBenchmark = (
  config?: Parameters<typeof performanceBenchmark.runFullBenchmarkSuite>[0],
) => performanceBenchmark.runFullBenchmarkSuite({ includeLongRunningTests: true, ...config });

export const benchmarkRedisVsClerk = (
  config?: Parameters<typeof performanceBenchmark.benchmarkRedisVsClerk>[0],
) => performanceBenchmark.benchmarkRedisVsClerk(config);

export const validateSub30msResponse = (
  config?: Parameters<typeof performanceBenchmark.validateSub30msResponse>[0],
