  ...
  try {
    ...
    return NextResponse.json({
      ...
      analysis: {
        ...
        patternAnalysis: config.patterns.map(
          (pattern: {
            name: string;
            ...
            expectedHitRatio:
              pattern.accessPattern === 'hotspot'
                ? 0.8
                : pattern.accessPattern === 'realistic'
                  ? 0.65
                  : 0.5,
  ...
  try {
    ...
    return NextResponse.json({
      ...
      concurrencyAnalysis: {
        ...
        concurrencyImpact:
          report.summary.percentiles.p95 > 100
            ? 'high'
            : report.summary.percentiles.p95 > 50
              ? 'medium'
              : 'low',
      },
    });
...
export async function runLoadTest(request: NextRequest) {
  try {
    ...
    const rampUpOps = report.benchmarks.filter((b) => b.metadata?.loadTestPhase === 'ramp_up');
    const steadyStateOps = report.benchmarks.filter(
      (b) => b.metadata?.loadTestPhase === 'steady_state',
    );
    ...
    const loadAnalysis = {
      rampUpPhase: {
        ...
        averageResponseTime:
          rampUpOps.reduce((sum, b) => sum + b.responseTime, 0) / rampUpOps.length || 0,
        errorRate: (rampUpOps.filter((b) => !b.success).length / rampUpOps.length) * 100,
      },
      steadyState: {
        operations: steadyStateOps.length,
        averageResponseTime:
          steadyStateOps.reduce((sum, b) => sum + b.responseTime, 0) / steadyStateOps.length || 0,
    ...
    return NextResponse.json({
      ...
      resilienceAnalysis: {
        ...
        circuitBreakerEffectiveness:
          report.benchmarks.filter((b) => b.metadata?.circuitBreakerTriggered).length > 0
            ? 'effective'
            : 'needs_review',
      },
    });
  ...
  try {
    ...
    const executiveSummary = {
      ...
      riskAssessment: {
        ...
        reliability:
          report.summary.failedOperations / report.summary.totalOperations > 0.01
            ? 'medium'
            : 'low',
        scalability:
          report.summary.cacheStats?.hitRatio && report.summary.cacheStats.hitRatio > 0.8
            ? 'low'
            : 'medium',
      },
    };
