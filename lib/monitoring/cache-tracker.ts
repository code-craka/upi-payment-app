...
class EnhancedCacheTracker {
  ...
  public async get<T = string>(
    ...
    try {
      ...
      const value = await withTimeoutAndDegradation(() => redis.get(key), {
  ...
  public async del(
    ...
    try {
      const result = await withTimeoutAndDegradation(() => redis.del(key), {
        operationConfig: { type: 'fast', service: 'redis' },
        enableGracefulDegradation: true,
  ...
  public async exists(
    ...
    try {
      const result = await withTimeoutAndDegradation(() => redis.exists(key), {
        operationConfig: { type: 'fast', service: 'redis' },
        enableGracefulDegradation: true,
    ...
    try {
      ...
      if (operation.keyPattern) {
        ...
        if (operation.abTestGroup) {
          ...
          pipeline.zadd(
            `cache:ab_test:${operation.abTestGroup}:latency`,
            timestamp,
            operation.latency,
          );
        }
    ...
    for (const group of groups) {
      ...
      const averageLatency =
        latencyValues.length > 0
          ? latencyValues.map((v) => parseFloat(v as string)).reduce((a, b) => a + b, 0) /
 ...
 */
export function withUserContext(userRole: string, userId?: string, source?: string) {
  return {
    get: <T = string>(key: string, options: CacheTrackingOptions = {}) =>
