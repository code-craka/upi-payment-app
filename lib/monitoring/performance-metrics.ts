...
class AdvancedPerformanceMetrics {
  ...
  private initializeBuffers(): void {
    const bufferKeys = [
      'latency:get', 'latency:set', 'latency:del', 'latency:exists',
      'size:keys', 'size:values', 'operations:success', 'operations:error'
    ];
  ...
  public recordMetric(
    ...
    this.persistMetricToRedis(operation, latency, success, timestamp, dataSize)
      .catch(error => console.error('Failed to persist metric:', error));
  }
  ...
  private async persistMetricToRedis(
    ...
    try {
      ...
      pipeline.zadd(`perf:latency:${operation}:5m`, { score: Math.floor(timestamp / 300000) * 300000, member: latency });
  ...
  public async getPerformanceMetrics(
    ...
    try {
      ...
      const hitRatio = totalOperations > 0 
        ? operationCounts.success / totalOperations 
        : 0;
  ...
  public async generatePerformanceAlerts(): Promise<PerformanceAlert[]> {
    try {
      ...
      if (metrics.latencyPercentiles.p95 > thresholds.p95Latency.emergency) {
        alerts.push(this.createAlert('emergency', 'p95_latency', 
          thresholds.p95Latency.emergency, metrics.latencyPercentiles.p95,
        ...
        alerts.push(this.createAlert('critical', 'p95_latency',
          thresholds.p95Latency.critical, metrics.latencyPercentiles.p95,
          'P95 latency exceeds critical threshold'));
      } else if (metrics.latencyPercentiles.p95 > thresholds.p95Latency.warning) {
        alerts.push(this.createAlert('warning', 'p95_latency',
          thresholds.p95Latency.warning, metrics.latencyPercentiles.p95,
          'P95 latency elevated above normal levels'));
      }
      
      // Check error rate
      if (metrics.errors.errorRate > thresholds.errorRate.emergency) {
        alerts.push(this.createAlert('emergency', 'error_rate',
          thresholds.errorRate.emergency, metrics.errors.errorRate,
        ...
        alerts.push(this.createAlert('critical', 'error_rate',
          thresholds.errorRate.critical, metrics.errors.errorRate,
          'Error rate exceeds acceptable levels'));
      }
      
      // Check hit ratio
      if (metrics.efficiency.hitRatio < thresholds.hitRatio.emergency) {
        alerts.push(this.createAlert('emergency', 'hit_ratio',
          thresholds.hitRatio.emergency, metrics.efficiency.hitRatio,
        ...
        alerts.push(this.createAlert('critical', 'hit_ratio',
          thresholds.hitRatio.critical, metrics.efficiency.hitRatio,
  ...
  private async getLatencyData(startTime: number, endTime: number): Promise<Record<string, number[]>> {
  ...
  private async getOperationCounts(startTime: number, endTime: number): Promise<{success: number; error: number}> {
    // Implementation would aggregate operation counts from Redis
    // For now, return mock data
    return { success: 1000, error: 50 };
  }
  
  private async getDataSizes(startTime: number, endTime: number): Promise<{
  ...
  private calculateEfficiencyScore(efficiency: PerformanceMetrics['efficiency']): number {
    ...
    if (efficiency.hitRatio >= 0.60) return 50;
    if (efficiency.hitRatio >= 0.40) return 25;
    return 10;
  }
...
export const getPerformanceReport = (timeWindow?: Parameters<AdvancedPerformanceMetrics['getPerformanceMetrics']>[0]) =>
