...
import { performanceMetrics, type PerformanceMetrics, type PerformanceTrend } from './performance-metrics';
...
class CacheTrendAnalyzer {
  ...
  public async analyzeMetricTrends(
    ...
    try {
      const [latencyTrends, throughputData, hitRatioData, errorRateData, memoryData] = await Promise.all([
  ...
  private async analyzeLatencyTrends(
    ...
    for (let time = now - timeWindow; time <= now; time += granularityMs) {
      try {
        ...
        const latencyValues = await this.getLatencyValuesForTimeWindow(
          time - granularityMs,
          time
        );
  ...
  private async analyzeThroughputTrend(
    ...
    for (let time = now - timeWindow; time <= now; time += granularityMs) {
      try {
        const operations = await this.getOperationCountForTimeWindow(
          time - granularityMs,
          time
        );
   ...
   */
  private async analyzeMemoryTrend(
    duration: string,
    granularity: string
  ): Promise<TrendAnalysis> {
    const timeWindow = this.getTimeWindow(duration);
    ...
    for (let time = now - timeWindow; time <= now; time += granularityMs) {
      try {
        ...
        const memoryUsage = await this.getMemoryUsageForTimeWindow(
          time - granularityMs,
          time
        );
   ...
   */
  private async analyzeTrendSeries(
    data: TrendPoint[],
    metricName: string
  ): Promise<TrendAnalysis> {
    if (data.length === 0) {
  ...
  private calculateStatistics(values: number[]): Omit<TrendAnalysis['statistics'], 'seasonality'> {
    if (values.length === 0) {
      return {
        ...
        min: 0, max: 0, trend: 'stable', trendStrength: 0
      };
    }
  ...
  private generateForecast(data: TrendPoint[]): TrendAnalysis['forecast'] {
    ...
    const trendAdjustment = trend === 'increasing' ? trendStrength : 
                           trend === 'decreasing' ? -trendStrength : 0;
  ...
  private generateInsights(
    ...
    if (statistics.trend === 'increasing') {
      if (metricName.toLowerCase().includes('latency') || metricName.toLowerCase().includes('error')) {
        patterns.push(`${metricName} is trending upward (${(statistics.trendStrength * 100).toFixed(1)}% strength)`);
      ...
      if (metricName.toLowerCase().includes('hit ratio') || metricName.toLowerCase().includes('throughput')) {
        patterns.push(`${metricName} is declining (${(statistics.trendStrength * 100).toFixed(1)}% strength)`);
    ...
    // Generate summary
    const summary = `${metricName} analysis: ${statistics.trend} trend with ${riskAssessment} risk level. ` +
   ...
   */
  private async getLatencyValuesForTimeWindow(startTime: number, endTime: number): Promise<number[]> {
    try {
  ...
  private async getOperationCountForTimeWindow(startTime: number, endTime: number): Promise<number> {
    try {
    ...
    try {
      ...
      return {
        nextHour: {
          ...
          expectedThroughput: { value: 520, confidence: 0.80 },
          expectedHitRatio: { value: 0.88, confidence: 0.90 },
          riskFactors: [
            {
...
export const analyzeTrends = (duration?: Parameters<CacheTrendAnalyzer['analyzeMetricTrends']>[0]) =>
