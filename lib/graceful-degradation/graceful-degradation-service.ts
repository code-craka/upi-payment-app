...
class GracefulDegradationService {
  ...
  private createTimeoutPromise(
    ...
    return new Promise((_, reject) => {
      const timeoutId = setTimeout(() => {
        reject(
          new TimeoutError(`Operation timed out after ${timeoutMs}ms`, operationConfig, timeoutMs),
        );
      }, timeoutMs);
   ...
   */
  private async executeFallbackStrategies<T>(strategies: FallbackStrategy<T>[]): Promise<T> {
  ...
  private async recordSuccess(serviceName: string, duration: number): Promise<void> {
    ...
    if (status) {
      ...
      if (
        ...
        duration < this.timeoutConfig.performanceBudget.p95Threshold
      ) {
  ...
  private async recordFailure(
    ...
    if (
      status &&
      status.consecutiveFailures >= this.timeoutConfig.circuitBreaker.failureThreshold
    ) {
  ...
  public async healthCheck(): Promise<{
    ...
    const fallbackSuccessRate =
      this.metrics.fallbacks.total > 0
