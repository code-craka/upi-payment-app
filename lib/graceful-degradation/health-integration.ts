...
class HealthIntegrationService {
    ...
    const services = {
      redis:
        redisHealth.status === 'fulfilled'
          ? redisHealth.value
          : this.createFailedServiceHealth('redis', redisHealth.reason),
      clerk:
        clerkHealth.status === 'fulfilled'
          ? clerkHealth.value
          : this.createFailedServiceHealth('clerk', clerkHealth.reason),
      database:
        dbHealth.status === 'fulfilled'
          ? dbHealth.value
          : this.createFailedServiceHealth('database', dbHealth.reason),
      auth:
        authHealth.status === 'fulfilled'
          ? authHealth.value
          : this.createFailedServiceHealth('auth', authHealth.reason),
      performance:
        perfHealth.status === 'fulfilled'
          ? perfHealth.value
          : this.createFailedServiceHealth('performance', perfHealth.reason),
    };
   ...
   * Get circuit breaker state from Redis directly
   */
  private async getCircuitBreakerStateFromRedis(
    service: string,
  ...
  private async calculateSystemMetrics() {
    ...
    try {
      ...
      const [responseTime, errorRate, cacheHitRatio, fallbackUsage] = await Promise.all([
        redis
          .get(responseTimeKey)
          .then((val) => (val ? parseFloat(val as string) : 0))
          .catch(() => 0),
        redis
          .get(errorRateKey)
          .then((val) => (val ? parseFloat(val as string) : 0))
          .catch(() => 0),
        redis
          .get(cacheHitRatioKey)
          .then((val) => (val ? parseFloat(val as string) : 0.8))
          .catch(() => 0.8),
        redis
          .get(fallbackUsageKey)
          .then((val) => (val ? parseFloat(val as string) : 0))
          .catch(() => 0),
      ]);
