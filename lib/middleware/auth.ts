...
export function isPublicRoute(
  pathname: string,
  publicRoutes: string[] = defaultAuthConfig.publicRoutes,
): boolean {
...
export function isAdminRoute(
  pathname: string,
  adminRoutes: string[] = defaultAuthConfig.adminRoutes,
): boolean {
 ...
 */
export async function getUserRoleWithFallback(user: { id: string; publicMetadata?: { role?: string } }): Promise<{
  role: UserRole | null;
  source: 'redis' | 'clerk';
  confidence: number;
  cached: boolean;
...
export async function requireRole(user: { id: string; publicMetadata?: { role?: string } }, requiredRole: UserRole): Promise<boolean> {
...
export async function requirePermission(user: { id: string; publicMetadata?: { role?: string; permissions?: string[] } }, permission: string): Promise<boolean> {
  if (!user) return false;
...
): Promise<{ user: { id: string; publicMetadata?: Record<string, unknown> } } | { error: NextResponse }> {
  const user = await getSafeUser();
