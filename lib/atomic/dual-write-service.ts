...
export class DualWriteRoleUpdateService {
  ...
  async executeRoleUpdate(
    ...
    try {
      ...
      const rollbackData = {
        roleData: currentRole ? JSON.stringify({
          userId: request.userId,
          role: currentRole,
      ...
      if (!redisResult.success) {
        ...
        await DualWriteRoleUpdateService.rollbackClerkUpdate(request.userId, clerkUser.publicMetadata, operationId)
        throw new AtomicOperationError(
          'Redis update failed',
      ...
      if (transaction.clerkResult?.success) {
        try {
          await DualWriteRoleUpdateService.rollbackClerkUpdate(request.userId, undefined, operationId)
  ...
  async executeBatchRoleUpdate(
    ...
    for (const request of requests) {
      try {
        const result = await this.executeRoleUpdate(
          request,
          initiatedBy,
          initiatedByEmail
        )
  ...
  private async logAuditEntry(entry: AtomicAuditLogEntry): Promise<void> {
    try {
      ...
      await redisCircuitBreaker.execute(() =>
        this.redis.lpush(auditKey, JSON.stringify(entry))
      )
      ...
      await redisCircuitBreaker.execute(() =>
        this.redis.expire(auditKey, ATOMIC_TTL.AUDIT_LOG)
      )
    } catch (error) {
  ...
  async getAuditLog(
    ...
    try {
      ...
      let auditEntries = entries.map(entry =>
        JSON.parse(entry as string) as AtomicAuditLogEntry
      )

      // Filter if needed
      if (userId) {
        auditEntries = auditEntries.filter(entry =>
          entry.userId === userId || entry.targetUserId === userId
        )
      }
